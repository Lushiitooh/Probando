...
function format(input) {
    // Traducción al español (scripts/es.js). Se devuelve tal cual: el español
    // no capitaliza cada palabra, así que no debe pasar por el formateo de abajo.
    // Si un id no está traducido, sigue el camino original en inglés.
    if (typeof ES_NOMBRES !== 'undefined' && ES_NOMBRES[input] !== undefined) return ES_NOMBRES[input];
...
function setWildPkmn(){
    ...
    document.getElementById(`training-indicator-counter`).textContent = `Restante: ${currentTrainingWave}`
    ...
    document.getElementById(`spiraling-highest-floor`).innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48"><defs><mask id="SVGIz3eBe9X"><g fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="4"><path d="M8 44h8m-4 0V4"/><path fill="#555555" d="M40 6H12v16h28l-4-8z"/></g></mask></defs><path fill="currentColor" d="M0 0h48v48H0z" mask="url(#SVGIz3eBe9X)"/></svg>Planta más alta: ${saved.maxSpiralFloor}`
...
function leaveCombat(){
    ...
    document.getElementById("area-end-item-title").innerHTML = "¡Objetos nuevos!"
    ...
    if (saved.currentArea == areas.frontierSpiralingTower.id){ //text
        ...
        if (saved.spiralRewardsClaimed >= 50) {
            setTimeout(() => {
                div.innerHTML = `No quedan recompensas. ¿Y si intentas batir tu récord?`
    ...
    if (saved.currentArea == areas.frontierBattleFactory.id){
        ...
        if (battleFactoryScore < minimumScore){
            setTimeout(() => {
                div.innerHTML = `Alcanza una puntuación mínima de ${minimumScore} para conseguir recompensas`
                document.getElementById("area-end-moves-title").appendChild(div);
                document.getElementById("area-end-moves-title").style.display = "flex"          
            }, 1);
            ...
            setTimeout(() => {
                div2.innerHTML = `Siguiente recompensa en ${nextRewardThreshold} score`
    ...
    for (i in pkmn) {//new moves notification
        ...
        const div = document.createElement("span");
        div.innerHTML = `${format(i)} ha aprendido ${joinWithAnd(pkmn[i].newMoves)}!`
    ...
    for (i in pkmn) {//new pokemon notification
        ...
        const div = document.createElement("span");
        div.innerHTML = `${format(i)} se ha desbloqueado!`
    ...
    if (noItems && noPkmn) {
        document.getElementById("area-end-item-title").innerHTML = "Ningún objeto ni Pokémon nuevo :("
        if (saved.currentArea == areas.training.id) document.getElementById("area-end-item-title").innerHTML = `<div style="display:flex; flex-direction:column; width: 100%; height:auto; justify-content:center; align-items:center; flex-shrink:0; text-align:center">Entrenamiento fallido :(<br><br>Intenta mejorar más a tu Pokémon:<br> <div class="genetics-overview-tags" > <div style="filter:hue-rotate(0deg)">Consigue movimientos que encajen mejor por tipo</div> <div style="filter:hue-rotate(0deg)">Consigue mejores movimientos que encajen con tu reparto de características (físico/especial)</div> <div style="filter:hue-rotate(0deg)">Usa movimientos que suban características acordes a tus movimientos</div> <div style="filter:hue-rotate(0deg)">Desbloquea una habilidad oculta</div> <div style="filter:hue-rotate(0deg)">Consigue una habilidad mejor</div> <div style="filter:hue-rotate(0deg)">Modifica las características con la Genética</div> </div></div>`
        document.getElementById("area-end-item-title").style.display = "flex"
    }

    //if (areas[saved.currentArea]?.trainer && areas[saved.currentArea]?.defeated) document.getElementById("area-end-item-title").innerHTML = "Las recompensas se han añadido automáticamente"
...
function updateWildPkmn(){
    ...
    // El respawn va por setTimeout (tiempo real), no por el bucle, así que hay
    // que acelerarlo aparte o sería el cuello de botella del x10.
    if (typeof velocidadCombate === 'function') respawnTimer /= velocidadCombate()
...
function openMenu(){
    ...
    if (saved.firstTimePlaying){
        ...
        if (esBrave) alert("¡Desactiva el escudo de Brave para que la página funcione bien!");
        });
        }
...
function returnItemLevel(id, mod) {
    ...
    if (mod == "left") {
        ...
        return `(${left} para el siguiente nivel)`;
    }
...
// Con el acelerador, el bucle ejecuta varios pasos de lógica por frame. Pintar
// las barras en cada uno satura el frame y limita la velocidad real, así que
// solo se pinta el último paso de cada frame: el jugador ve lo mismo.
let pasoVisible = true;
...
function gameLoop(now) {
    ...
    // Acelerador de combate: al meter más tiempo del que ha pasado de verdad,
    // el while de abajo ejecuta esa misma proporción de pasos de más. Todo lo
    // que depende del bucle se acelera igual, así que no se descuadra nada.
    accumulator += delta * (typeof velocidadCombate === 'function' ? velocidadCombate() : 1);

    let stepsExecuted = 0;

    // Cuántos pasos toca ejecutar en este frame; sirve para saber cuál es el
    // último y pintar solo en ese.
    const pasosPrevistos = Math.min(Math.floor(accumulator / STEP), MAX_STEPS_PER_FRAME);
...
function exploreCombatPlayer() {
    ...
    if (afkSeconds > 0 || !pasoVisible) { //afk time / paso intermedio del acelerador
    ...
    if (barProgressPlayer < 99) {
        ...
        if (nextMove.id == move.metronome.id) {
            ...
            console.info(`Metrónomo lanza: `+format(selectedMove))
        ...
        // Modificador "Cruce constante": da por bueno el cruce aunque el
        // movimiento anterior fuera del mismo tipo (o no hubiera anterior).
        const cruceValido = saved.gamemodCruce == true
            || (lastCrossStab!=undefined && lastCrossStab!=move[nextMovePlayer].type)

        //on successful cross move
        if (cruceValido && /*pkmn[team[exploreActiveMember].pkmn.id].type.includes(move[nextMovePlayer].type) &&*/ move[nextMovePlayer].power>0 && ( !testAbility(`active`, "ate") ||  move[nextMovePlayer].type!=="normal" ) ) {
        ...
        for (const member in team) {
            ...
            * 1;
            ...
            if (afkSeconds<10){
            console.info(`Movimiento Z ejecutado: infligió ${zTotalPower.toFixed(0)} de daño`)
            if (zUser.type.includes(zTyping)) console.info(`- Multiplicador STAB del movimiento Z: sí (x1.5)`)
            if (zSplit == `random`) console.info(`- Bonus de reparto aleatorio del movimiento Z: sí (x1.25)`)
            console.info(`- Multiplicador de tipo enemigo del movimiento Z: ${zTypeMultiplier}`)
            }
        ...
        if (  saved.gamemodFatiga == true) fatigueDamage = 0
...
function setWildAreas() {
    ...
    <span >
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd" d="m17.861 3.163l.16.054l1.202.4c.463.155.87.29 1.191.44c.348.162.667.37.911.709s.341.707.385 1.088c.04.353.04.781.04 1.27v8.212c0 .698 0 1.287-.054 1.753c-.056.484-.182.962-.535 1.348a2.25 2.25 0 0 1-.746.538c-.478.212-.971.18-1.448.081c-.46-.096-1.018-.282-1.68-.503l-.043-.014c-1.12-.374-1.505-.49-1.877-.477a2.3 2.3 0 0 0-.441.059c-.363.085-.703.299-1.686.954l-1.382.922l-.14.093c-1.062.709-1.8 1.201-2.664 1.317c-.863.116-1.705-.165-2.915-.57l-.16-.053l-1.202-.4c-.463-.155-.87-.29-1.191-.44c-.348-.162-.667-.37-.911-.71c-.244-.338-.341-.706-.385-1.088c-.04-.353-.04-.78-.04-1.269V8.665c0-.699 0-1.288.054-1.753c.056-.484.182-.962.535-1.348a2.25 2.25 0 0 1 .746-.538c.478-.213.972-.181 1.448-.081c.46.095 1.018.282 1.68.503l.043.014c1.12.373 1.505.49 1.878.477a2.3 2.3 0 0 0 .44-.059c.363-.086.703-.3 1.686-.954l1.382-.922l.14-.094c1.062-.708 1.8-1.2 2.663-1.316c.864-.116 1.706.165 2.916.57m-2.111.943V16.58c.536.058 1.1.246 1.843.494l.125.042c.717.239 1.192.396 1.555.472c.356.074.477.04.532.016a.75.75 0 0 0 .249-.179c.04-.044.11-.149.152-.51c.043-.368.044-.869.044-1.624V7.163c0-.54-.001-.88-.03-1.138c-.028-.239-.072-.328-.112-.382c-.039-.054-.109-.125-.326-.226c-.236-.11-.56-.218-1.07-.389l-1.165-.388c-.887-.296-1.413-.464-1.797-.534m-1.5 12.654V4.434c-.311.18-.71.441-1.276.818l-1.382.922l-.11.073c-.688.46-1.201.802-1.732.994v12.326c.311-.18.71-.442 1.276-.819l1.382-.921l.11-.073c.688-.46 1.201-.802 1.732-.994m-6 3.135V7.42c-.536-.058-1.1-.246-1.843-.494l-.125-.042c-.717-.239-1.192-.396-1.556-.472c-.355-.074-.476-.041-.53-.017a.75.75 0 0 0-.25.18c-.04.043-.11.148-.152.509c-.043.368-.044.87-.044 1.625v8.128c0 .54.001.88.03 1.138c.028.239.072.327.112.382c.039.054.109.125.326.226c.236.11.56.218 1.07.389l1.165.388c.887.295 1.412.463 1.797.534" clip-rule="evenodd"/></svg>    Zonas Salvajes
    </span>
    ...
    Rotación ${currentWildRotation}/${rotationWildMax}</strong>
    ...
    let seasonTimer = `Zona limitada hasta ${season[saved.currentSeason].end.month}/${season[saved.currentSeason].end.day}`
    if (saved.temporalSeason != undefined) seasonTimer = `Zona limitada hasta ${saved.temporalSeason.end.month}/${saved.temporalSeason.end.day}`
    ...
    divPark.innerHTML = `
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="currentColor" d="M25.719 4.781a2.9 2.9 0 0 0-1.125.344l-4.719 2.5L13.5 6.062l-.375-.093l-.375.187l-2.156 1.25l-1.281.75l1.187.906l2.719 2.063l-3.406 1.813l-3.657-1.657l-.437-.187l-.438.219l-1.75.937l-1.156.625l.875.938l5.406 5.812l.5.594l.688-.375L15 17.094l-1.031 5.687l-.344 1.813l1.719-.719l2.562-1.094l.375-.156l.157-.375l3.718-9.031l5.25-2.813c1.446-.777 2.028-2.617 1.25-4.062a3 3 0 0 0-1.781-1.438a3.1 3.1 0 0 0-1.156-.125m.187 2c.125-.008.254-.004.375.032a.979.979 0 0 1 .188 1.812l-5.594 3.031l-.313.156l-.125.344l-3.718 8.938l-.438.187l1.063-5.906l.375-2.031l-1.813.969l-6.312 3.406l-3.969-4.313l.156-.094l3.657 1.626l.468.218l.406-.25l15.22-8.031a.9.9 0 0 1 .374-.094M13.375 8.094l3.844.937l-2.063 1.063l-2.25-1.719zM3 26v2h26v-2z"/></svg>
                        ...
                        <span><strong style="background:#B18451">Nivel: ${Math.max(1,areas.wildlifePark.level-10)}-${areas.wildlifePark.level}</strong><span></span></span>
                    </span>
                </div>
                <div style="width: 8rem;" class="explore-ticket-right">
    ...
    if (shinyMark==true) completedFlair = `<svg style="color: rgb(209, 130, 11); opacity: 0.7" class="completed-flair" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15"><path fill="currentColor" d="M14 1v1.5c-.75 0-.75 1.5 0 1.5v1.25c-.75 0-.75 1.5 0 1.5v1.5c-.75 0-.75 1.5 0 1.5V11c-.75 0-.75 1.5 0 1.5V14h-1.5c0-.75-1.5-.75-1.5 0H9.75c0-.75-1.5-.75-1.5 0h-1.5c0-.75-1.5-.75-1.5 0H4c0-.75-1.5-.75-1.5 0H1v-1.5c.75 0 .75-1.5 0-1.5V9.75c.75 0 .75-1.5 0-1.5v-1.5c.75 0 .75-1.5 0-1.5V4c.75 0 .75-1.5 0-1.5V1h1.5c0 .75 1.5.75 1.5 0h1.25c0 .75 1.5.75 1.5 0h1.5c0 .75 1.5.75 1.5 0H11c0 .75 1.5.75 1.5 0zm-2 2H3v9h9zm-1 1v7H4V4z"/></svg>`
        ...
        divAreas.innerHTML = `
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="currentColor" d="M25.719 4.781a2.9 2.9 0 0 0-1.125.344l-4.719 2.5L13.5 6.062l-.375-.093l-.375.187l-2.156 1.25l-1.281.75l1.187.906l2.719 2.063l-3.406 1.813l-3.657-1.657l-.437-.187l-.438.219l-1.75.937l-1.156.625l.875.938l5.406 5.812l.5.594l.688-.375L15 17.094l-1.031 5.687l-.344 1.813l1.719-.719l2.562-1.094l.375-.156l.157-.375l3.718-9.031l5.25-2.813c1.446-.777 2.028-2.617 1.25-4.062a3 3 0 0 0-1.781-1.438a3.1 3.1 0 0 0-1.156-.125m.187 2c.125-.008.254-.004.375.032a.979.979 0 0 1 .188 1.812l-5.594 3.031l-.313.156l-.125.344l-3.718 8.938l-.438.187l1.063-5.906l.375-2.031l-1.813.969l-6.312 3.406l-3.969-4.313l.156-.094l3.657 1.626l.468.218l.406-.25l15.22-8.031a.9.9 0 0 1 .374-.094M13.375 8.094l3.844.937l-2.063 1.063l-2.25-1.719zM3 26v2h26v-2z"/></svg>
                        ...
                        <span><strong style="background:#B18451">Nivel: ${Math.max(1,areas[i].level-10)}-${areas[i].level}</strong><span></span></span>
                    </span>
                </div>
                <div style="width: 8rem;" class="explore-ticket-right">
...
function setDungeonAreas() {
    ...
    <div class="rotation-timer">
    <strong><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M6.94 2c.416 0 .753.324.753.724v1.46c.668-.012 1.417-.012 2.26-.012h4.015c.842 0 1.591 0 2.259.013v-1.46c0-.4.337-.725.753-.725s.753.324.753.724V4.25c1.445.111 2.394.384 3.09 1.055c.698.67.982 1.582 1.097 2.972L22 9H2v-.724c.116-1.39.4-2.302 1.097-2.972s1.645-.944 3.09-1.055V2.724c0-.4.337-.724.753-.724"/><path fill="currentColor" d="M22 14v-2c0-.839-.004-2.335-.017-3H2.01c-.013.665-.01 2.161-.01 3v2c0 3.771 0 5.657 1.172 6.828S6.228 22 10 22h4c3.77 0 5.656 0 6.828-1.172S22 17.772 22 14" opacity="0.5"/><path fill="currentColor" d="M18 17a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0"/></svg>
    Rotación ${rotationDungeonCurrent}/${rotationDungeonMax}</strong>
    <div class="time-counter-daily"></div>
    </div>
       ...
       <span>${areas[i].unlockDescription}</span>
        ...
        divAreas.innerHTML = `
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="currentColor" d="M25.719 4.781a2.9 2.9 0 0 0-1.125.344l-4.719 2.5L13.5 6.062l-.375-.093l-.375.187l-2.156 1.25l-1.281.75l1.187.906l2.719 2.063l-3.406 1.813l-3.657-1.657l-.437-.187l-.438.219l-1.75.937l-1.156.625l.875.938l5.406 5.812l.5.594l.688-.375L15 17.094l-1.031 5.687l-.344 1.813l1.719-.719l2.562-1.094l.375-.156l.157-.375l3.718-9.031l5.25-2.813c1.446-.777 2.028-2.617 1.25-4.062a3 3 0 0 0-1.781-1.438a3.1 3.1 0 0 0-1.156-.125m.187 2c.125-.008.254-.004.375.032a.979.979 0 0 1 .188 1.812l-5.594 3.031l-.313.156l-.125.344l-3.718 8.938l-.438.187l1.063-5.906l.375-2.031l-1.813.969l-6.312 3.406l-3.969-4.313l.156-.094l3.657 1.626l.468.218l.406-.25l15.22-8.031a.9.9 0 0 1 .374-.094M13.375 8.094l3.844.937l-2.063 1.063l-2.25-1.719zM3 26v2h26v-2z"/></svg>
                        ...
                        <span style="font-size:1.2rem">${format(i)}</span>
                        <span><strong style="background:#6BBC77">Nivel: ${Math.max(1,areas[i].level-10)}-${areas[i].level}</strong><span></span></span>
                    </span>
                </div>
                <div style="width: 8rem;" class="explore-ticket-right">
                    <span class="explore-ticket-bg" style="background-image: url(img/bg/${areas[i].background}.png);"></span>
...
function setEventAreas() {
    ...
    if (saved.tutorialStep !== "none") {
        ...
        document.getElementById("tooltipMid").innerHTML = `Completa el tutorial para acceder a los eventos`
        openTooltip()
        return
    }
    ...
    if (eventCategory==2 && areas.vsTeamLeaderGiovanni.defeated!=true){
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Derrota al Líder de Equipo Giovanni en modo VS para desbloquear`
        openTooltip()
        return
    }
    ...
    if (eventCategory==3){
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Aún no está implementado`
        openTooltip()
        return
    }
    ...
    if (rotationEventCurrent==1) eventTitle = `Regreso a Kanto`
    if (rotationEventCurrent==2) eventTitle = `Parajes Primigenios`
    if (rotationEventCurrent==3) eventTitle = `Despertar de los Ancestros`
    if (rotationEventCurrent==4) eventTitle = `Toma de Aether`
    if (rotationEventCurrent==5) eventTitle = `Ciencia del Futuro`
    if (rotationEventCurrent==6) eventTitle = `Expedición a Sinnoh`
    ...
    <div class="rotation-timer">
    <strong><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M6.94 2c.416 0 .753.324.753.724v1.46c.668-.012 1.417-.012 2.26-.012h4.015c.842 0 1.591 0 2.259.013v-1.46c0-.4.337-.725.753-.725s.753.324.753.724V4.25c1.445.111 2.394.384 3.09 1.055c.698.67.982 1.582 1.097 2.972L22 9H2v-.724c.116-1.39.4-2.302 1.097-2.972s1.645-.944 3.09-1.055V2.724c0-.4.337-.724.753-.724"/><path fill="currentColor" d="M22 14v-2c0-.839-.004-2.335-.017-3H2.01c-.013.665-.01 2.161-.01 3v2c0 3.771 0 5.657 1.172 6.828S6.228 22 10 22h4c3.77 0 5.656 0 6.828-1.172S22 17.772 22 14" opacity="0.5"/><path fill="currentColor" d="M18 17a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0"/></svg>
    Rotación ${rotationEventCurrent}/${rotationEventMax}</strong>
  ...
  for (const i in areas) {
       ...
       if (areas[i].encounter && areas[i].difficulty===tier2difficulty && areas.vsEliteFourLance.defeated!=true) unlockRequirement =`<span class="ticket-unlock">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M2 16c0-2.828 0-4.243.879-5.121C3.757 10 5.172 10 8 10h8c2.828 0 4.243 0 5.121.879C22 11.757 22 13.172 22 16s0 4.243-.879 5.121C20.243 22 18.828 22 16 22H8c-2.828 0-4.243 0-5.121-.879C2 20.243 2 18.828 2 16" opacity="0.5"/><path fill="currentColor" d="M6.75 8a5.25 5.25 0 0 1 10.5 0v2.004c.567.005 1.064.018 1.5.05V8a6.75 6.75 0 0 0-13.5 0v2.055a24 24 0 0 1 1.5-.051z"/></svg>
       Derrota al Alto Mando Lance en VS para desbloquear</span>`
       ...
       if (areas[i].encounter && areas[i].difficulty===tier4difficulty && areas.vsUltraEntityLusamine.defeated!=true) unlockRequirement =`<span class="ticket-unlock">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M2 16c0-2.828 0-4.243.879-5.121C3.757 10 5.172 10 8 10h8c2.828 0 4.243 0 5.121.879C22 11.757 22 13.172 22 16s0 4.243-.879 5.121C20.243 22 18.828 22 16 22H8c-2.828 0-4.243 0-5.121-.879C2 20.243 2 18.828 2 16" opacity="0.5"/><path fill="currentColor" d="M6.75 8a5.25 5.25 0 0 1 10.5 0v2.004c.567.005 1.064.018 1.5.05V8a6.75 6.75 0 0 0-13.5 0v2.055a24 24 0 0 1 1.5-.051z"/></svg>
       Derrota a la Entidad Ultra Lusamine en VS para desbloquear</span>`


       let eventTag ;
       eventTag = `<strong class="event-tag">Zona Salvaje ✦</strong>`
       if (areas[i].uncatchable) eventTag = `<strong style="filter:hue-rotate(140deg)" class="event-tag">Colección ◈</strong>`
       if (areas[i].encounter && areas[i].difficulty===tier1difficulty) eventTag = `<strong style="filter:hue-rotate(50deg)" class="event-tag">Incursión Nivel I ❖</strong>`
       if (areas[i].encounter && areas[i].difficulty===tier2difficulty) eventTag = `<strong style="filter:hue-rotate(200deg)" class="event-tag">Incursión Nivel II ❖</strong>`
       if (areas[i].encounter && areas[i].difficulty===tier3difficulty) eventTag = `<strong style="filter:hue-rotate(300deg)" class="event-tag">Incursión Nivel III ❖</strong>`
       if (areas[i].encounter && areas[i].difficulty===tier4difficulty) eventTag = `<strong style="filter:hue-rotate(100deg)" class="event-tag">Incursión Nivel IV ❖</strong>`
       ...
       if (areas[i].name) eventName = areas[i].name
        ...
        divAreas.innerHTML = `
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="currentColor" d="M25.719 4.781a2.9 2.9 0 0 0-1.125.344l-4.719 2.5L13.5 6.062l-.375-.093l-.375.187l-2.156 1.25l-1.281.75l1.187.906l2.719 2.063l-3.406 1.813l-3.657-1.657l-.437-.187l-.438.219l-1.75.937l-1.156.625l.875.938l5.406 5.812l.5.594l.688-.375L15 17.094l-1.031 5.687l-.344 1.813l1.719-.719l2.562-1.094l.375-.156l.157-.375l3.718-9.031l5.25-2.813c1.446-.777 2.028-2.617 1.25-4.062a3 3 0 0 0-1.781-1.438a3.1 3.1 0 0 0-1.156-.125m.187 2c.125-.008.254-.004.375.032a.979.979 0 0 1 .188 1.812l-5.594 3.031l-.313.156l-.125.344l-3.718 8.938l-.438.187l1.063-5.906l.375-2.031l-1.813.969l-6.312 3.406l-3.969-4.313l.156-.094l3.657 1.626l.468.218l.406-.25l15.22-8.031a.9.9 0 0 1 .374-.094M13.375 8.094l3.844.937l-2.063 1.063l-2.25-1.719zM3 26v2h26v-2z"/></svg>
                        ...
                        <span><strong style="background:#D57392">Nivel: ${levelrange}</strong>${eventTag}<span></span></span>
                    </span>
                </div>
                <div style="width: 8rem;" class="explore-ticket-right">
                    <span class="explore-ticket-bg" style="background-image: url(img/bg/${areas[i].background}.png);"></span>
...
function updateRaidTimer(){
    ...
    document.getElementById("raid-timer-indicator-current").innerHTML = `Tiempo restante: ${formattedTime}`
...
function updatePokedex(){
    ...
    if (dexTeamSelect != undefined) {
        ...
        document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon para añadir al equipo`
    }

    if (tmToTeach != undefined) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon al que enseñar ${format(tmToTeach)}`
    }

    if (memoryToTeach != undefined) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon al que enseñar ${format(memoryToTeach)}`
    }

    if (evoItemToUse != undefined) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon para usar el ${format(evoItemToUse)}`
    }

    if (vitaminToUse != undefined) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon para usar el ${format(vitaminToUse)}`
    }

    if (itemToUse != undefined) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon para usar el ${format(itemToUse)}`
    }

    if (dexHostSelect == true) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon anfitrión`
    }

    if (dexTrainSelect == true) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon al que entrenar`
    }

    if (dexSampleSelect == true) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon muestra`
    }
        ...
        */
        ...
        if (evoItemToUse != undefined ) {
            ...
            div.addEventListener("click", e => { 
                ...
                document.getElementById("tooltipBottom").innerHTML = `${format(pkmn[ pkmn[i].evolve()[evo].pkmn.id ].id)} se ha desbloqueado!`
                openTooltip()
                updateItemBag()
        ...
        givePkmn(pkmn[ pkmn[i].evolve()[1].pkmn.id ],1)
            ...
            if (itemToUse == item.heartScale.id){
                ...
                document.getElementById("tooltipBottom").style.display = "none" 
                document.getElementById("tooltipTitle").innerHTML = `Elige el movimiento que recordar`
                document.getElementById("tooltipTop").style.display = "none"    
                ...
                No hay movimientos nuevos que recordar
                `
            ...
            if (itemToUse == item.abilityPatch.id){
                ...
                document.getElementById("tooltipTitle").innerHTML = `¡Habilidad nueva!`
                document.getElementById("tooltipTop").style.display = "none"    
...
function tagMenuAssign(){
    ...
    if (!saved.tagSystemTags || saved.tagSystemTags.length === 0) {
        document.getElementById("tag-listing").innerHTML = 'Aún no has creado etiquetas';
        return;
    }
...
function updateEditorTags(){
    if (pkmn[currentEditedPkmn].tagList?.length > 0) {
        ...
        document.getElementById(`tag-system-editor-tags`).innerHTML = `Pulsa aquí para añadir etiquetas` // No tags, show prompt
        return
    }
...
function tagMenuRemove(id){
    document.getElementById("tooltipBottom").style.display = "none"
    document.getElementById("tooltipMid").innerHTML = `<div id="remove-tag-menu" style="display:flex; flex-direction:column">¿Seguro que quieres borrar esta etiqueta? Se quitará de todos los Pokémon</div>`
    const div = document.createElement(`div`)
    div.className = `tag-system-tag`
    ...
    document.getElementById("tooltipMid").innerHTML = `
        ...
        <div class="tag-system-form-section">
            <div class="tag-system-form-group">
                <label for="tag-system-tagName">Nombre de la etiqueta</label>
                <input type="text" id="tag-system-tagName" placeholder="Escribe el nombre de la etiqueta...">
            </div>
            ...
            <div class="tag-system-form-group">
                <label for="tag-system-tagColor">Color de la etiqueta</label>
            ...
            <button class="tag-system-save-btn" onclick="tagSystemSaveTag()">Guardar etiqueta</button>
        </div>
    </div>
    `
        ...
        function tagSystemSaveTag() {
            ...
            if (!tagName) {
                alert('Escribe un nombre para la etiqueta');
                return;
            }
 
            if (!tagSystemSelectedIcon) {
                alert('Elige un icono');
                return;
            }
        ...
        function tagSystemRenderTags() {
            ...
            if (saved.tagSystemTags.length === 0) {
                tagsList.innerHTML = '<div class="tag-system-empty-state">Aún no has creado etiquetas. ¡Crea la primera arriba!</div>';
                return;
            }
...
function switchMenu(id){
    ...
    if (/vs|items|team|dex|dictionary|guide/.test(id) && saved.tutorialStep != "none") {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Completa antes el tutorial`
        openTooltip()
        return
    }
    ...
    if (id=="shop" && areas.vsGymLeaderBrock.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Derrota al Líder de Gimnasio Brock en modo VS para desbloquear`
        openTooltip()
        return
    }
    ...
    if (id=="dimension" && areas.vsLegendTrainerBrendan.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Derrota al Entrenador Legendario Brendan en modo VS para desbloquear`
        openTooltip()
        return
    }
    ...
    if (id=="genetics" && areas.vsEliteFourLance.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Derrota al Alto Mando Lance en modo VS para desbloquear`
        openTooltip()
        return
    }
    ...
    if (id=="training" && areas.vsGymLeaderMisty.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Derrota a la Líder de Gimnasio Misty en modo VS para desbloquear`
        openTooltip()
        return
    }
    ...
    //if (id=="team") document.getElementById(`preview-team-exit`).textContent = "Guardar y salir"
    //else document.getElementById(`preview-team-exit`).textContent = "¡Guardar y salir!"
...
function updateItemBag(){
    ...
    for (const i in item) {
        ...
        if (item[i].type == "tm" && dexTeamSelect==undefined) {
            ...
            document.getElementById("pokedex-filters-title").style.display = "flex"
            document.getElementById("pokedex-filters-cancel").style.display = "flex"
            document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon al que enseñar ${format(item[i].move)}`
            document.getElementById("menu-button-parent").style.display = "none"
        ...
        if (item[i].type == "memory" && dexTeamSelect==undefined) {
            ...
            document.getElementById("pokedex-filters-title").style.display = "flex"
            document.getElementById("pokedex-filters-cancel").style.display = "flex"
            document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon al que enseñar ${format(item[i].ability)}`
            document.getElementById("menu-button-parent").style.display = "none"
            document.getElementById(`item-menu`).style.display = "none"
        ...
        if (item[i].evo && dexTeamSelect==undefined) {
            ...
            document.getElementById("pokedex-filters-title").style.display = "flex"
            document.getElementById("pokedex-filters-cancel").style.display = "flex"
            document.getElementById("pokedex-filters-title").innerHTML = `Elige un Pokémon para usar el ${format(i)}`
            document.getElementById("menu-button-parent").style.display = "none"
            document.getElementById(`item-menu`).style.display = "none"
...
function updateVS() {
 ...
 for (const i in areas) {
    ...
    if (areas[i].defeated) continue;
       ...
       let fieldTag = ""
       if (areas[i].fieldEffect) fieldTag = `<strong style="font-size:1rem; background:#725AA4; margin-left:0.2rem">+ Efecto de campo</strong>`
        ...
        divAreas.innerHTML = `
                ...
                <div class="vs-card-bg"></div>
                    <span class="explore-ticket-left" style="z-index: 2;">
                        ...
                        <span><strong style="font-size:1rem; background:#465f96">Nivel ${Math.max(1,(areas[i].level))}</strong>${fieldTag}</span>
                    </span>
                <div>
                </div>
...
function updateFrontier() {
    ...
    let divisionText = ``
    if (rotationFrontierCurrent==1) divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}Copa Menor${cupsvg}</span><div>${returnDivisionLetter("C")} solo división y por debajo</div>`
    if (rotationFrontierCurrent==2) divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}Liga Grande${cupsvg}</span><div>${returnDivisionLetter("B")} solo división y por debajo</div>`
    if (rotationFrontierCurrent==3) divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}Liga Ultra${cupsvg}</span><div>${returnDivisionLetter("A")} solo división y por debajo</div>`
    if (rotationFrontierCurrent==4) divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}Liga Maestra${cupsvg}</span><div>${returnDivisionLetter("SSS")} solo división y por debajo</div>`
    ...
    <div class="rotation-timer">
    <strong><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M6.94 2c.416 0 .753.324.753.724v1.46c.668-.012 1.417-.012 2.26-.012h4.015c.842 0 1.591 0 2.259.013v-1.46c0-.4.337-.725.753-.725s.753.324.753.724V4.25c1.445.111 2.394.384 3.09 1.055c.698.67.982 1.582 1.097 2.972L22 9H2v-.724c.116-1.39.4-2.302 1.097-2.972s1.645-.944 3.09-1.055V2.724c0-.4.337-.724.753-.724"/><path fill="currentColor" d="M22 14v-2c0-.839-.004-2.335-.017-3H2.01c-.013.665-.01 2.161-.01 3v2c0 3.771 0 5.657 1.172 6.828S6.228 22 10 22h4c3.77 0 5.656 0 6.828-1.172S22 17.772 22 14" opacity="0.5"/><path fill="currentColor" d="M18 17a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0"/></svg>
    Rotación ${rotationFrontierCurrent}/${rotationFrontierMax}</strong>
    <div class="time-counter-event"></div>
    </div>

    <div class="rotation-timer">
    <strong><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M6.94 2c.416 0 .753.324.753.724v1.46c.668-.012 1.417-.012 2.26-.012h4.015c.842 0 1.591 0 2.259.013v-1.46c0-.4.337-.725.753-.725s.753.324.753.724V4.25c1.445.111 2.394.384 3.09 1.055c.698.67.982 1.582 1.097 2.972L22 9H2v-.724c.116-1.39.4-2.302 1.097-2.972s1.645-.944 3.09-1.055V2.724c0-.4.337-.724.753-.724"/><path fill="currentColor" d="M22 14v-2c0-.839-.004-2.335-.017-3H2.01c-.013.665-.01 2.161-.01 3v2c0 3.771 0 5.657 1.172 6.828S6.228 22 10 22h4c3.77 0 5.656 0 6.828-1.172S22 17.772 22 14" opacity="0.5"/><path fill="currentColor" d="M18 17a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0"/></svg>
    Reinicio de entrenadores</strong>
    <div class="time-counter-daily"></div>
    </div>
    </div>
    ...
    spiral.innerHTML = `
        ...
        <span class="explore-ticket-left">
        <span style="font-size:1.2rem">Torre de Combate</span>
    ...
    factory.innerHTML = `
        ...
        <span class="explore-ticket-left">
        <span style="font-size:1.2rem">Fábrica de Combate</span>
        <span><strong style="background:#964646ff">Mejor puntuación: ${saved.maxFactoryScore}</strong><span></span></span>
        </span>
        </div>
        <div style="width: 8rem;" class="explore-ticket-right">
    ...
    arena.innerHTML = `
        ...
        <span class="explore-ticket-left">
        <span style="font-size:1.2rem">Arena de Combate</span>
...
const tagBurn = `<span data-buff="burn"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("fire")}">Quemadura</span></span>`
const tagFreeze = `<span data-buff="freeze"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("ice")}">Congelación</span></span>`
const tagParalysis = `<span data-buff="paralysis"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("electric")}">Parálisis</span></span>`
const tagPoisoned = `<span data-buff="poisoned"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("poison")}">Envenenado</span></span>`
const tagSleep = `<span data-buff="sleep"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("normal")}">Dormido</span></span>`
const tagConfused = `<span data-buff="confused"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("psychic")}">Confuso</span></span>`
...
const tagSunny = `<span data-buff="sunny"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("fire")}">Soleado</span></span>`
const tagRainy = `<span data-buff="rainy"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("water")}">Lluvia</span></span>`
const tagSandstorm = `<span data-buff="sandstorm"><span style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("ground")}">Tormenta de Arena</span></span>`
const tagHail = `<span data-buff="hail"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("ice")}">Granizo</span></span>`
const tagFoggy = `<span data-buff="foggy"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("ghost")}">Niebla</span></span>`
const tagElectricTerrain = `<span data-buff="electricTerrain"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("electric")}">Campo Eléctrico</span></span>`
const tagGrassyTerrain = `<span data-buff="grassyTerrain"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("grass")}">Campo de Hierba</span></span>`
const tagMistyTerrain = `<span data-buff="mistyTerrain"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("fairy")}">Campo de Niebla</span></span>`


const tagTrickRoom = `<span data-buff="trickRoom"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("dark")}">Espacio Raro</span></span>`
const tagWeirdRoom = `<span data-buff="weirdRoom"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("bug")}">Zona Extraña</span></span>`
const tagCrossRoom = `<span data-buff="crossRoom"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("fighting")}">Zona Cruzada</span></span>`
...
const tagLightScreen = `<span data-buff="lightScreen"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("fairy")}">Pantalla de Luz</span></span>`
const tagSafeguard = `<span data-buff="safeguard"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("ice")}">Velo Sagrado</span></span>`
...
function claimExportReward(){
        ...
        document.getElementById("tooltipTitle").innerHTML = `Recompensa recibida`
        document.getElementById("tooltipMid").style.display = `none`
...
let geneticItemSelect = false
    ...
    document.getElementById("genetics-start").addEventListener("click", e => {
        ...
        document.getElementById("tooltipTop").style.display = `none`
        document.getElementById("tooltipTitle").innerHTML = `¡Operación terminada!<br>¿Quieres usar un objeto de genética?`
        document.getElementById("tooltipMid").innerHTML = `El objeto se consumirá al usarlo`
        ...
        document.getElementById("tooltipTop").style.display = `none`
        document.getElementById("tooltipTitle").innerHTML = `¿Seguro que quieres abortar la operación?`
        document.getElementById("tooltipMid").innerHTML = `Solo perderás el tiempo invertido`
...
if (saved.geneticOperation !== undefined){
    document.getElementById("genetics-start").textContent = "Abortar"
    ...
    document.getElementById("genetics-start").textContent = "Empezar"
...
if (saved.geneticOperation <= 1){
    document.getElementById("genetics-start").textContent = "Terminar"
...
if (mod==="end"){
    ...
    if (rng(shinyChance)) {pkmn[saved.geneticHost].shiny = true; summaryTags += `<div style="filter:hue-rotate(100deg)">✦ ¡Mutación variocolor!</div>` }
    ...
    summaryTags += `<div style="filter:hue-rotate(-50deg)">★ ¡Habilidad intercambiada!</div>`
    } else {
    const newAbility = learnPkmnAbility(saved.geneticHost,10) //boosted chance
    if (itemUsed=="everstone") {pkmn[saved.geneticHost].ability = newAbility; summaryTags += `<div style="filter:hue-rotate(-50deg)">★ Habilidad nueva: ${format(newAbility)}!</div>`}
    }
    ...
    pkmn[saved.geneticSample].movepool.forEach(moveID => {
        if (rng(moveChance) && !(pkmn[saved.geneticHost].movepool.includes(moveID)) && (move[moveID].moveset!==undefined ||  (   move[moveID].moveset==undefined && pkmn[saved.geneticHost].eggMove?.id == moveID  )    ||   ( move[moveID].moveset==undefined && ["B", "C", "D"].includes(returnPkmnDivision(pkmn[saved.geneticHost])) && item.replicatorUpgradeE.got>0 && compability>=3   )   ) ) {
            ...
            if (move[moveID].moveset==undefined && (pkmn[saved.geneticSample].signature?.id == moveID || pkmn[saved.geneticSample].eggMove?.id == moveID)  ) summaryTags += `<div style="filter:hue-rotate(200deg)">⟐ Movimiento de huevo heredado: ${format(moveID)}!</div>`
            else summaryTags += `<div style="filter:hue-rotate(0deg)">◇ Movimiento heredado: ${format(moveID)}!</div>`
        }
    });
    ...
    if (itemUsed==`lockCapsule`){
        ...
        summaryTags += `<div style="filter:hue-rotate(-200deg)">★ ¡Movimientos transferidos!</div>`
    ...
    if (rng(ivChanceHp) && pkmn[saved.geneticHost].ivs.hp<Math.min(ivCap, pkmn[saved.geneticSample].ivs.hp)) {pkmn[saved.geneticHost].ivs.hp = Math.min(ivCap, pkmn[saved.geneticSample].ivs.hp) ; summaryTags += `<div style="filter:hue-rotate(200deg)">❖ ¡IV de PS heredados!</div>`}
    if (rng(ivChanceAtk) && pkmn[saved.geneticHost].ivs.atk<Math.min(ivCap, pkmn[saved.geneticSample].ivs.atk)) {pkmn[saved.geneticHost].ivs.atk = Math.min(ivCap, pkmn[saved.geneticSample].ivs.atk) ; summaryTags += `<div style="filter:hue-rotate(200deg)">❖ ¡IV de Ataque heredados!</div>`}
    if (rng(ivChanceDef) && pkmn[saved.geneticHost].ivs.def<Math.min(ivCap, pkmn[saved.geneticSample].ivs.def)) {pkmn[saved.geneticHost].ivs.def = Math.min(ivCap, pkmn[saved.geneticSample].ivs.def) ; summaryTags += `<div style="filter:hue-rotate(200deg)">❖ ¡IV de Defensa heredados!</div>`}
    if (rng(ivChanceSatk) && pkmn[saved.geneticHost].ivs.satk<Math.min(ivCap, pkmn[saved.geneticSample].ivs.satk)) {pkmn[saved.geneticHost].ivs.satk = Math.min(ivCap, pkmn[saved.geneticSample].ivs.satk) ; summaryTags += `<div style="filter:hue-rotate(200deg)">❖ ¡IV de At. Esp. heredados!</div>`}
    if (rng(ivChanceSdef) && pkmn[saved.geneticHost].ivs.sdef<Math.min(ivCap, pkmn[saved.geneticSample].ivs.sdef)) {pkmn[saved.geneticHost].ivs.sdef = Math.min(ivCap, pkmn[saved.geneticSample].ivs.sdef) ; summaryTags += `<div style="filter:hue-rotate(200deg)">❖ ¡IV de Def. Esp. heredados!</div>`}
    if (rng(ivChanceSpe) && pkmn[saved.geneticHost].ivs.spe<Math.min(ivCap, pkmn[saved.geneticSample].ivs.spe)) {pkmn[saved.geneticHost].ivs.spe = Math.min(ivCap, pkmn[saved.geneticSample].ivs.spe) ; summaryTags += `<div style="filter:hue-rotate(200deg)">❖ ¡IV de Velocidad heredados!</div>`}
    ...
    for (const iv in pkmn[saved.geneticHost].ivs){
        ...
        if (newIv>ivId) {
            ...
            if (iv === "hp") summaryTags += `<div style="filter:hue-rotate(250deg)">◆ ¡IV de PS aumentados!</div>`
            if (iv === "atk") summaryTags += `<div style="filter:hue-rotate(250deg)">◆ ¡IV de Ataque aumentados!</div>`
            if (iv === "def") summaryTags += `<div style="filter:hue-rotate(250deg)">◆ ¡IV de Defensa aumentados!</div>`
            if (iv === "satk") summaryTags += `<div style="filter:hue-rotate(250deg)">◆ ¡IV de At. Esp. aumentados!</div>`
            if (iv === "sdef") summaryTags += `<div style="filter:hue-rotate(250deg)">◆ ¡IV de Def. Esp. aumentados!</div>`
            if (iv === "spe") summaryTags += `<div style="filter:hue-rotate(250deg)">◆ ¡IV de Velocidad aumentados!</div>`
        }
    }


    if (summaryTags == "") summaryTags = "Sin cambios genéticos nuevos"
...
setInterval(() => {
    ...
    if (saved.geneticOperation<1 && document.getElementById("genetics-start").textContent == "Abortar") setGeneticMenu();
...
training.level = {
    name: `Entrenamiento de nivel`,
    info: `Sube un Pokémon al nivel máximo. Solo si está por debajo del nivel 100`,
    tier: 2,
    color: `#dfc969`,
    errorText: `Derrota a la Entrenadora de Élite Cynthia en modo VS para desbloquear, o ya has alcanzado el nivel máximo`,
    ...
    effect: function() {
        ...
        setTimeout(() => {
        const div = document.createElement("span");
        div.innerHTML = `${format(saved.trainingPokemon)} ha subido al nivel ${pkmn[saved.trainingPokemon].level}!`
        document.getElementById("area-end-moves-title").appendChild(div);
        document.getElementById("area-end-moves-title").style.display = "flex"
...
training.iv1 = { //disapears if you have more than x ivs
    name: `Entrenamiento de IV I`,
    info: `Gana 2 estrellas de IV al azar. Solo con menos de 10 estrellas de IV`,
    tier: 1,
    color: `#699edf`,
    condition: function() {
    ...
    errorText: `Debes tener menos de 10 estrellas de IV, o ya has llegado al máximo`,
    effect: function() {
    ...
    let text = `¡Aumentado ${parts.slice(0, -1).join(", ")} y ${parts.at(-1)}!`
    if (parts.length === 1) text = `Aumentado ${parts[0]}!`;
    pkmn[i].dictionaryTagIvSum = pkmn[i].ivs.hp + pkmn[i].ivs.atk + pkmn[i].ivs.satk + pkmn[i].ivs.spe + pkmn[i].ivs.sdef + pkmn[i].ivs.def
...
training.iv2 = { //doesnt appear until you have more than x ivs
    name: `Entrenamiento de IV II`,
    info: `Gana 2 estrellas de IV al azar. Solo con menos de 22 estrellas de IV`,
    tier: 2,
    color: `#699edf`,
    condition: function() {
        const id = pkmn[saved.trainingPokemon]
    ...
    errorText: `Debes tener menos de 22 estrellas de IV, o ya has llegado al máximo`,
    effect: function() {
...
training.iv3 = { //doesnt appear until you have more than x ivs
    name: `Entrenamiento de IV III`,
    info: `Gana 2 estrellas de IV al azar`,
    tier: 3,
    color: `#699edf`,
    condition: function() {
        const id = pkmn[saved.trainingPokemon]
    ...
    errorText: `Ya tienes los IV al máximo posible`,
    effect: function() {
...
training.ability = {
    name: `Entrenamiento de habilidad`,
    info: `Vuelve a sortear la habilidad del Pokémon`,
    tier: 1,
    color: `#69df96`,
    effect: function() {
        ...
        setTimeout(() => {
        const div = document.createElement("span");
        div.innerHTML = `${format(saved.trainingPokemon)} ahora tiene ${format(newAbility)}!`
        document.getElementById("area-end-moves-title").appendChild(div);
        document.getElementById("area-end-moves-title").style.display = "flex"
        document.getElementById("area-end-item-title").style.display = "none"
        }, 10);
...
training.hiddenAbility = {
    name: `Hidden Entrenamiento de habilidad`,
    info: `Desbloquea la habilidad oculta del Pokémon`,
    tier: 2,
    color: `#69df96`,
    ...
    errorText: `Ya has desbloqueado la habilidad oculta de este Pokémon`,
    effect: function() {
        pkmn[saved.trainingPokemon].hiddenAbilityUnlocked = true

        setTimeout(() => {
        const div = document.createElement("span");
        div.innerHTML = `${format(saved.trainingPokemon)} ahora tiene ${format(pkmn[saved.trainingPokemon].hiddenAbility.id)}!`
        document.getElementById("area-end-moves-title").appendChild(div);
        document.getElementById("area-end-moves-title").style.display = "flex"
        document.getElementById("area-end-item-title").style.display = "none"
        }, 10);
...
training.move = { //disapears if you have 20+ moves or smth
    name: `Entrenamiento de movimientos`,
    info: `Aprende un movimiento nuevo. Solo con menos de 20 movimientos, o cuando haya uno nuevo disponible`,
    tier: 1,
    color: `#cf79c1`,
    ...
    errorText: `Ya has aprendido más de 20 movimientos, o no hay más que aprender a este nivel`,
    effect: function() {
        let learntMove = learnPkmnMove(pkmn[saved.trainingPokemon].id, pkmn[saved.trainingPokemon].level)
...
training.nature = {
    name: `Entrenamiento de naturaleza`,
    info: `Concede, resortea y elimina naturalezas, que modifican las estrellas de características: <br><br>Firme: Atk ▲, At.Esp ▼<br>Modesta: At.Esp ▲, Atk ▼<br>Alegre: Vel ▲, Def ▼, Def.Esp ▼<br>Relajada: PS ▲, Vel ▼<br>Mansa: PS ▲, Atk ▼, At.Esp ▼<br>Osada: Def ▲, Def.Esp ▲, PS ▼<br><br>Firme y Modesta no pueden salir si subieran la característica ofensiva más alta del Pokémon, y ninguna naturaleza puede superar 6 estrellas ni dejar la Velocidad en 0`,
    tier: 3,
    color: `#DF7A69`,
    condition: function() { if (areas.vsLegendTrainerBrendan.defeated == true) return true },
    errorText: `Derrota al Entrenador Legendario Brendan en modo VS para desbloquear`,
    effect: function() {
        ...
        setTimeout(() => {
        const div = document.createElement("span");
        div.innerHTML = `${format(saved.trainingPokemon)} ahora tiene un ${format(pickedNature)} nature!`
        document.getElementById("area-end-moves-title").appendChild(div);
        document.getElementById("area-end-moves-title").style.display = "flex"
        document.getElementById("area-end-item-title").style.display = "none"
        }, 10);
...
function setTrainingMenu() {
    ...
    if (restrictedError) {
        document.getElementById("tooltipTop").style.display = "none"
        document.getElementById("tooltipBottom").style.display = "none"
        document.getElementById("tooltipTitle").innerHTML = `Movimientos restringidos`
        document.getElementById("tooltipMid").innerHTML = `El Pokémon en entrenamiento tiene varios movimientos restringidos (${restrictedIcon}) equipped!`
        openTooltip()
        return
    }
...
function battleSummary() {
    ...
    document.getElementById("tooltipTop").style.display = `none`
    document.getElementById("tooltipTitle").innerHTML = `Daño infligido`
    document.getElementById("tooltipBottom").style.display = `none`
...
const mysteryGift = {
    ...
    info: `¡Pulsación larga o clic derecho en el regalo de abajo para recibir un Kecleon!<br>Será variocolor y llevará una Cinta Recuerdo`,
...
function claimWonderTrade(){
    ...
    if (areas.vsMasterTrainerGeeta.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Derrota a la Entrenadora Maestra Geeta en modo VS para desbloquear`
        openTooltip()
        return
    }
    ...
    document.getElementById("tooltipTop").style.display = "none"
    document.getElementById("tooltipTitle").innerHTML = `Intercambio Milagroso`
    document.getElementById("tooltipMid").innerHTML = `Cada 12 h puedes recibir un Pokémon al azar`
    document.getElementById("tooltipBottom").innerHTML = `

        <div onclick="wonderTrade()" class="custom-challenge-button" style="margin-top:0.5rem">¡Vamos allá!</div>
...
function wonderTrade(){
    ...
    document.getElementById("wonder-text").innerHTML = `Gracias por el ${format(chosenPokemon)}!`
...
function seasonCheck() {
    ...
    if (newSeason) { //executes once per season, resets shop
        console.info(`${format(saved.currentSeason)} ha empezado la temporada, ¡tiendas reabastecidas!`)
...
function renamePokemon(){
    document.getElementById("tooltipTop").style.display = "none"
    document.getElementById("tooltipBottom").style.display = "none"
    document.getElementById("tooltipTitle").innerHTML = "¡Ponle un mote!"
    document.getElementById("tooltipMid").innerHTML = `
...
function updateMegaDimension(tier){
    ...
    <strong><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M6.94 2c.416 0 .753.324.753.724v1.46c.668-.012 1.417-.012 2.26-.012h4.015c.842 0 1.591 0 2.259.013v-1.46c0-.4.337-.725.753-.725s.753.324.753.724V4.25c1.445.111 2.394.384 3.09 1.055c.698.67.982 1.582 1.097 2.972L22 9H2v-.724c.116-1.39.4-2.302 1.097-2.972s1.645-.944 3.09-1.055V2.724c0-.4.337-.724.753-.724"/><path fill="currentColor" d="M22 14v-2c0-.839-.004-2.335-.017-3H2.01c-.013.665-.01 2.161-.01 3v2c0 3.771 0 5.657 1.172 6.828S6.228 22 10 22h4c3.77 0 5.656 0 6.828-1.172S22 17.772 22 14" opacity="0.5"/><path fill="currentColor" d="M18 17a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0"/></svg>
    Rotación ${rotationDimensionCurrent}/${rotationDimensionMax}</strong>
       ...
       <span>${areas[i].unlockDescription}</span>
        ...
        divAreas.innerHTML = `
                ...
                <span class="explore-ticket-left">
                        ...
                        <span><strong style="background:#9F50B5">Nivel: ${Math.max(1,areas[i].level-10)}-${areas[i].level}</strong><span></span></span>
                    </span>
                </div>
                <div style="width: 8rem;" class="explore-ticket-right">
                    <span class="explore-ticket-bg" style="background-image: url(img/bg/${areas[i].background}.png);"></span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ajustes añadidos: loadGame() reemplaza `saved` entero, así que las
    // partidas antiguas llegan aquí sin estas claves.
    saved.velocidadCombate ??= 1;
    saved.gamemodExp ??= false;
    saved.gamemodDrops ??= false;
    saved.gamemodShiny ??= false;
