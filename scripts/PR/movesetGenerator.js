...
(function () {
    ...
    async function loadSVGIcon(name, color = "#fff", size = 18) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`No se pudo cargar ${name}: "${response.status} ${response.statusText}`);
            }
            ...
            console.error(`[MovesetGenerator] No se pudo cargar icon: ${name}`, error);
            return "";
        }
    }
                ...
                <tr ${rowBgStyle} data-split="${splitName}">
                    ...
                    <td class="col-split">
                        <span class="split-trigger" data-split-trigger="${splitName}" 
                              title="${splitName ? `Filter by ${capitalizedSplit} moves` : "Sin categoría"}">
    ...
    async function showMovePopup(pkmnObj, initialLevel) {
        ...
        levelInput.setAttribute("aria-label", "Nivel del Pokémon");
        levelInput.min = 1;
        levelInput.max = 100;
        ...
        async function updateContent(newLevel) {
            ...
            title.textContent = `Movimientos de ${format(report.pokemon)}`;
            ...
            for (const typeName of Object.keys(buckets.sameType)) {
                ...
                contentHTML += await buildTableHTML(
                    `Mismo tipo: ${capitalizedType}`,
                    buckets.sameType[typeName]
                );
            }
            contentHTML += await buildTableHTML("Coincidencias por etiqueta de repertorio", buckets.movesetMatch);
            contentHTML += await buildTableHTML("Movimientos de todos los tipos", buckets.allTag);
