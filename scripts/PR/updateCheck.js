...
async function getLatestCommit() {
  try {
    ...
    if (!res.ok) {
      console.warn(`Error de la API de GitHub: ${res.status}`);
      return null;
    }
    ...
    console.warn(`No se pudo obtener el último commit: ${err}`);
    return null;
  }
}
...
window.addEventListener("app-update-available", () => {
  ...
  banner.textContent = "Hay una actualización disponible";
