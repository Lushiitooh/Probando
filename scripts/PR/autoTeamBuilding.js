...
function openAutoTeam(){
    ...
    document.getElementById("tooltipTitle").innerHTML = "Autoconstrucción de equipo"
    ...
    for (let i = 1; i <= 6; i++) {
        ...
        //PR-EDIT
        const itemName = currentTeam[slotKey].item || 'Sin objeto'
        if (itemName == "Sin objeto") continue
        itemCheckboxesHTML += `
              <label style="display:flex; align-items:center; gap:.5rem; font-size:0.9rem;">
                ...
                Fijar ${format(itemName)} (ranura ${i})
              </label>`
    }

    document.getElementById("tooltipMid").innerHTML = `Elige la orientación del equipo (tu equipo actual será reemplazado)`
    document.getElementById("tooltipBottom").innerHTML = `
    <span id="prevent-tooltip-exit"></span>
        <div style="display:flex; flex-direction:column; gap:.35rem;">

              <span style="opacity:.8;">
                Izquierda = defensivo, derecha = ofensivo
              </span>
    ...
    <div onClick = '
        ...
        ' style="cursor:pointer; font-size:1.5rem; width:100%;" class="auto-build-confirm" id="prevent-tooltip-exit">Autoconstruir
    </div>
