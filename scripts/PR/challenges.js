// Retos personalizados - Create or import custom challenges
// by gwenillia
...
(function () {
  ...
  const parseIvLine = (line, mon, errors) => {
    ...
    ivLine.split("/").forEach(part => {
      ...
      if (!statKey) {
        errors.push(`Característica de IV desconocida: ${match[2].trim()}`);
      } else if (isNaN(value) || value < 0 || value > 6) {
        errors.push(`Valor de IV no válido para ${statKey}: ${value}`);
      } else {
  ...
  const parseChallengeText = (raw) => {
    ...
    lines.forEach(line => {
      ...
      if (/^ability:/i.test(trimmed)) {
        ...
        if (!abilityId) result.errors.push(`Habilidad desconocida: ${abilityName}`);
      ...
      if (/^level:/i.test(trimmed)) {
        ...
        if (isNaN(levelValue) || levelValue < 1) {
          result.errors.push(`Nivel no válido: ${trimmed}`);
        } else {
      ...
      if (/^-\s+/.test(trimmed)) {
        ...
        if (!moveId) {
          result.errors.push(`Movimiento desconocido: ${moveName}`);
        } else if (current.moves.length >= 4) {
          result.errors.push(`Demasiados movimientos para ${format(current.id)}.`);
        } else {
    ...
    if (result.enemyTeam.length < CONFIG.MIN_TEAM || result.enemyTeam.length > CONFIG.MAX_TEAM) {
      result.errors.push(`El equipo enemigo debe tener ${CONFIG.MIN_TEAM}-${CONFIG.MAX_TEAM} Pokémon.`);
    }
    [...result.playerTeam, ...result.enemyTeam].forEach(mon => {
      if (mon.moves.length === 0) {
        result.errors.push(`${format(mon.id)} necesita al menos un movimiento.`);
      }
    });
...
- Shadow Ball"
  ...
  const handleSave = () => {
    ...
    if (!rawTitle) {
      showError("Ponle un título a tu reto.");
      return;
    }

    if (!rawText) {
      showError("Escribe primero el texto del reto.");
      return;
    }
    ...
    const challenge = {
      ...
      title: title.value.trim() || parseResult.title || "Reto personalizado",
      ...
      actions: [
        {
          ...
          onClick: () => {
            ...
            try {
              ...
              if (errorEl) errorEl.textContent = "Código de reto no válido.";
              return;
            }
            ...
            saved.customChallenges.unshift({
              ...
              title: String(data?.title ?? parseResult.title ?? "Reto personalizado").trim(),
  ...
  const handleExport = (challenge) => {
    ...
    openChallengeDialog({
      ...
      body: `
        ...
        <div style=\"opacity:0.75; margin-top:0.4rem;\">Comparte este código para importar el reto.</div>
      `,
      actions: [
        {
          label: "Copy",
  ...
  const openPlayerSelection = (challenge, parseResult) => {
    ...
    document.getElementById("tooltipTitle").innerHTML = "Elige tu equipo";
    document.getElementById("tooltipMid").innerHTML = `
      <div id="custom-challenge-selection-count">Seleccionados 0/${CONFIG.MAX_TEAM}</div>
      <div style="opacity:0.8; margin-top:0.3rem;">Pick ${CONFIG.MIN_TEAM}-${CONFIG.MAX_TEAM} Pokémon del grupo del jugador.</div>
      ...
      entry.addEventListener("click", () => {
        ...
        if (selection.has(idx)) {
          ...
          if (selection.size >= CONFIG.MAX_TEAM) {
            if (errorEl) errorEl.textContent = `Solo puedes seleccionar ${CONFIG.MAX_TEAM} Pokémon.`;
            return;
          }
          selection.add(idx);
    ...
    startButton.textContent = "Empezar reto";
    startButton.style.cssText = "cursor:pointer; margin-top:0.6rem;";
    startButton.addEventListener("click", (e) => {
      e.stopPropagation();
      const errorEl = document.getElementById("custom-challenge-selection-error");
      if (selection.size < CONFIG.MIN_TEAM) {
        if (errorEl) errorEl.textContent = `Selecciona al menos ${CONFIG.MIN_TEAM} Pokémon.`;
        return;
      }
      if (errorEl) errorEl.textContent = "";
  ...
  const renderChallengeCard = (challenge, options = {}) => {
    ...
    teams.innerHTML = `
      <span><strong>Equipo:</strong> ${formatTeamPreview(challenge.playerTeam || [])}</span>
      <span><strong>Enemigo:</strong> ${formatTeamPreview(challenge.enemyTeam || [])}</span>
    `;
  ...
  const updateCustomChallenges = () => {
    ...
    if (!saved.customChallenges.length) {
      ...
      empty.textContent = "Aún no hay retos personalizados.";
  ...
  const updateMainChallenges = () => {
    ...
    if (!Array.isArray(window.mainChallenges) || !window.mainChallenges.length) {
      const empty = document.createElement("div");
      empty.className = "custom-challenge-card";
      empty.textContent = "Los retos principales llegarán pronto.";
      list.appendChild(empty);
      return;
    }
  ...
  const setChallengesTab = (tab) => {
    ...
    if (tab === "main") {
      ...
      if (headerTitle) headerTitle.textContent = "Retos personalizados";
      if (headerHelp) headerHelp.dataset.help = "Retos personalizados";
    }
  };
