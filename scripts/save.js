...
function exportToText() {
  const raw = localStorage.getItem("gameData");
  if (!raw) {
    console.log("No se encontró ninguna partida guardada");
    return null;
  }
...
function loadFromText() {
  ...
  if (!input) {
    alert("No se encontró el elemento con id 'text-data-raw'");
    return;
  }

  const jsonData = input.value.trim();
  if (!jsonData) {
    alert("No hay datos en el campo");
    return;
  }
...
async function pasteFromClipboard() {
  ...
  try {
    ...
    alert("No se pudo pegar desde el portapapeles. Pégalo a mano o concede permisos de portapapeles.");
  }
}
...
function textData() {
  ...
  if (savedData) {
    document.getElementById("tooltipMid").innerHTML = `
      Este es el código de tu partida guardada<br>Aquí puedes copiar o pegar códigos de partida para exportar o importar<br>
    ...
    document.getElementById("tooltipBottom").innerHTML = `
      <div style="display:flex;width:100%; align-items:center;justify-content:center; flex-wrap:wrap;">
        <div onClick='navigator.clipboard.writeText(document.getElementById("text-data-raw").value); alert("¡Datos copiados al portapapeles!");' 
    ...
    document.getElementById("tooltipMid").innerHTML = `
      Aquí puedes copiar o pegar códigos de partida para exportar o importar<br>
      <textarea id="text-data-raw" rows="10" style="width:95%; resize:vertical; font-family:monospace; font-size:0.9rem;"></textarea>
    `;
