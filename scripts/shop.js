...
shop.eventhalloweenTheme = {
    ...
    name: `Tema de temporada permanente`,
    price: 50,
    stock: 1,
...
function updateItemShop(){
    ...
    La decoración disponible rotará en <font style="margin-left:0.3rem" class="time-counter-event">...</font>`
    ...
    decorTimer.innerHTML =`
    <svg style="scale:0.7" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M12,1A11,11,0,1,0,23,12,11,11,0,0,0,12,1Zm0,20a9,9,0,1,1,9-9A9,9,0,0,1,12,21Z"/><rect width="2" height="7" x="11" y="6" fill="currentColor" rx="1"><animateTransform attributeName="transform" dur="450s" repeatCount="indefinite" type="rotate" values="0 12 12;360 12 12"/></rect><rect width="2" height="9" x="11" y="11" fill="currentColor" rx="1"><animateTransform attributeName="transform" dur="37.5s" repeatCount="indefinite" type="rotate" values="0 12 12;360 12 12"/></rect></svg>
    Los objetos disponibles rotarán en <font style="margin-left:0.3rem" class="time-counter-daily">...</font>`
    document.getElementById("shop-listing").appendChild(decorTimer);
    }
...
div.addEventListener("click", () => {
    ...
    document.getElementById("tooltipTop").style.display = "none"
    document.getElementById("tooltipTitle").innerHTML = "¿Cuántos quieres comprar?"
    ...
    function buyItem(amount) {
        ...
        if ( ( item[currency].got>=(shop[i].price*amount) && shop[i].stock == undefined ) || ( item[currency].got>=(shop[i].price*amount) && shop[i].stock >= amount ) ){
            ...
            document.getElementById("tooltipTitle").innerHTML = "No alcanza";
            document.getElementById("tooltipTop").style.display = "none"    
            document.getElementById("tooltipTop").style.display = "none"    
            document.getElementById("tooltipMid").style.display = "none"
            document.getElementById("tooltipBottom").innerHTML = `No te puedes permitir esta compra<span id="prevent-tooltip-exit"></span>`
        }
    }
...
function updateCurry(){
   ...
   if (saved.lastCurryRotation == rotationWildCurrent){
        ...
        document.getElementById(`curry-title`).innerHTML = `Vuelve en <span class="time-counter-daily"></span>`
        updateDailyCounters()
        return
    } else {
    ...
    document.getElementById(`curry-title`).innerHTML = `¡Elige ingredientes para el curry! [${curryIngredientList.length}/3]`
...
function makeCurry(){
    ...
    if (curryIngredientList.length==0){
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Elige al menos un ingrediente primero`
        openTooltip()
        return
    }
    ...
    if (item.goldenBottleCap.got < totalPrice){
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `No te lo puedes permitir`
        openTooltip()
        return
    }
    ...
    setTimeout(() => {
        ...
        document.getElementById("tooltipTop").style.display = `none`
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Durante las incursiones: todo tu equipo gana temporalmente estas habilidades:`
