...
function tooltipData(category, ttdata){
    ...
    if (category == "area") {
        ...
        document.getElementById("tooltipBottom").style.display = "inline";
        document.getElementById("tooltipBottom").innerHTML = `<div id="area-preview-spawns"><strong>Pokémon de la zona</strong></div>`;
        ...
        if (areas[ttdata].type == "dungeon") { document.getElementById("tooltipBottom").innerHTML = `<div id="area-preview-spawns"><strong>Pokémon de la zona ${uncatchableIcon}</strong></div>`; }
        if (areas[ttdata].uncatchable) { document.getElementById("tooltipBottom").innerHTML = `<div id="area-preview-spawns"><strong>Pokémon de la zona ${uncatchableIcon}</strong></div>`; }
        ...
        document.getElementById("tooltipMid").innerHTML = `<div id="area-preview-items"><strong>Objetos de la zona</strong></div>`;
        ...
        if (ttdata == areas.wildlifePark.id) {
            document.getElementById("tooltipBottom").innerHTML += `Los Pokémon del Parque Natural rotan cada 12 horas`
        }
    ...
    if (category == "trainer") {
        ...
        document.getElementById("tooltipMid").style.display = "none";
        document.getElementById("tooltipBottom").style.display = "inline";
        document.getElementById("tooltipBottom").innerHTML = `<div id="area-preview-spawns"><strong>Vista previa del equipo</strong></div><div id="preview-field-effects"></div>`;
        ...
        document.getElementById("tooltipMid").innerHTML = `<div id="area-preview-items"><strong>Recompensas de victoria</strong></div>`;
        ...
        if (areas[ttdata].itemReward) { //new reward function ill eventually have to convert the rest of the code but for now im quite lazy
            ...
            tooltipMid.innerHTML = `<div id="area-preview-items"><strong>Recompensas de victoria</strong></div>`;
        ...
        if (areas[ttdata].fieldEffect && areas[ttdata].type != "frontier") { 
            document.getElementById("preview-field-effects").innerHTML = `Efectos de campo`
        ...
        if (areas[ttdata].hpPercentage){
            document.getElementById("tooltipBottom").innerHTML += `Este es un encuentro cronometrado. Tu daño se conserva entre combates. Además, los objetos necesarios para entrar se consumen tanto si derrotas al Pokémon como si no`;
        }
    ...
    if (category == "buff") {
        ...
        document.getElementById("tooltipBottom").style.display = `inline`
        document.getElementById("tooltipTitle").style.display = `inline`
        if (ttdata==="burn") document.getElementById("tooltipBottom").innerHTML = `Reduce el Ataque Físico un 50% y hace daño cada turno`
        if (ttdata==="poisoned") document.getElementById("tooltipBottom").innerHTML = `Reduce el Ataque Especial un 50% y hace daño cada turno`
        if (ttdata==="sleep") document.getElementById("tooltipBottom").innerHTML = `Los movimientos no logran hacer daño`
        if (ttdata==="freeze") document.getElementById("tooltipBottom").innerHTML = `Los movimientos no logran hacer daño`
        if (ttdata==="confused") document.getElementById("tooltipBottom").innerHTML = `50% de probabilidad de que los movimientos no hagan daño`
        if (ttdata==="paralysis") document.getElementById("tooltipBottom").innerHTML = `25% de probabilidad de que los movimientos no hagan daño, y la Velocidad se reduce un 75%`

        if (ttdata==="embargo") document.getElementById("tooltipBottom").innerHTML = `Impide al enemigo usar el movimiento de la ranura en la que se usó Embargo`
        
        if (ttdata==="sunny") document.getElementById("tooltipBottom").innerHTML = `Aumenta un 50% el daño de los movimientos de tipo Fuego y reduce un 50% el de los de tipo Agua`
        if (ttdata==="rainy") document.getElementById("tooltipBottom").innerHTML = `Aumenta un 50% el daño de los movimientos de tipo Agua y reduce un 50% el de los de tipo Fuego`
        if (ttdata==="sandstorm") document.getElementById("tooltipBottom").innerHTML = `Aumenta un 50% el daño de los movimientos de tipo Roca y Tierra`
        if (ttdata==="hail") document.getElementById("tooltipBottom").innerHTML = `Aumenta un 50% el daño de los movimientos de tipo Hielo`
        if (ttdata==="foggy") document.getElementById("tooltipBottom").innerHTML = `Aumenta un 50% el daño de los movimientos de tipo Siniestro y Fantasma`
        if (ttdata==="electricTerrain") document.getElementById("tooltipBottom").innerHTML = `Aumenta un 50% el daño de los movimientos de tipo Eléctrico y Acero`
        if (ttdata==="grassyTerrain") document.getElementById("tooltipBottom").innerHTML = `Aumenta un 50% el daño de los movimientos de tipo Planta y Bicho`
        if (ttdata==="mistyTerrain") document.getElementById("tooltipBottom").innerHTML = `Aumenta un 50% el daño de los movimientos de tipo Hada y Psíquico`

        if (ttdata==="trickRoom") document.getElementById("tooltipBottom").innerHTML = `Los Pokémon lentos se vuelven rápidos y los rápidos, lentos. Además, multiplica el daño de x1.0 a x1.5 según lo lento que fuera el Pokémon originalmente`
        if (ttdata==="weirdRoom") document.getElementById("tooltipBottom").innerHTML = `Copia todas las características base de ataque y defensa del enemigo`
        if (ttdata==="crossRoom") document.getElementById("tooltipBottom").innerHTML = `Aumenta un 30% el daño de la Potencia Cruzada`
        //if (ttdata==="reflect") document.getElementById("tooltipBottom").innerHTML = `Reduce un 75% el daño de los ataques físicos`
        if (ttdata==="lightScreen") document.getElementById("tooltipBottom").innerHTML = `El daño supereficaz recibido por tu equipo se reduce a neutro`
        if (ttdata==="safeguard") document.getElementById("tooltipBottom").innerHTML = `Protege a tu equipo de los estados alterados y las bajadas de características`
    ...
    div.innerHTML = `
        ...
        <span><strong style="background:#964646ff">PS restantes: ${areas[i].hpPercentage.toFixed(0)}%</strong><span></span></span>
        </span>
        </div>
        <div style="width: 8rem;" class="explore-ticket-right">
    ...
    if (saved.arenaCurrentTrainer==6){
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Por ahora has derrotado a todos los entrenadores`
        openTooltip()
        return
    }
    ...
    Elige la dificultad del siguiente entrenador
    <span>(clic derecho o pulsación larga para ver los detalles del efecto de campo)</span>
    </div>
...
frontierArray.sort((a, b) => a.data.tier - b.data.tier);
    ...
    const i = obj.key;
       ...
       //if (areas[i].reward.includes(item.goldenBottleCap)) nameTag = `<svg class="event-icon" style="color:#465f96" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M12.795 2h-2c-1.886 0-2.829 0-3.414.586c-.586.586-.586 1.528-.586 3.414v3.5h10V6c0-1.886 0-2.828-.586-3.414S14.681 2 12.795 2" opacity="0.5"/><path fill="currentColor" fill-rule="evenodd" d="M13.23 5.783a3 3 0 0 0-2.872 0L5.564 8.397A3 3 0 0 0 4 11.031v4.938a3 3 0 0 0 1.564 2.634l4.794 2.614a3 3 0 0 0 2.872 0l4.795-2.614a3 3 0 0 0 1.564-2.634V11.03a3 3 0 0 0-1.564-2.634zM11.794 10.5c-.284 0-.474.34-.854 1.023l-.098.176c-.108.194-.162.29-.246.354s-.19.088-.399.135l-.19.044c-.739.167-1.108.25-1.195.532c-.088.283.163.577.666 1.165l.13.152c.144.167.215.25.247.354s.022.215 0 .438l-.02.203c-.076.785-.114 1.178.116 1.352s.575.015 1.266-.303l.179-.082c.196-.09.294-.135.398-.135s.203.045.399.135l.179.082c.69.319 1.036.477 1.266.303s.192-.567.116-1.352l-.02-.203c-.022-.223-.033-.334 0-.438c.032-.103.103-.187.246-.354l.13-.152c.504-.588.755-.882.667-1.165c-.088-.282-.457-.365-1.194-.532l-.191-.044c-.21-.047-.315-.07-.399-.135c-.084-.064-.138-.16-.246-.354l-.098-.176c-.38-.682-.57-1.023-.855-1.023" clip-rule="evenodd"/></svg>`
        let prefix = "Entrenador Novato "
        if (areas[i].tier==2) prefix = "Entrenador Veterano "
        if (areas[i].tier==3) prefix = "Entrenador Guay "
        if (areas[i].tier==4) prefix = "Entrenador Experto "
        if (areas[i].tier==5) prefix = "Entrenador Maestro "
    ...
    for (const i in areas[ttdata].skills) {
        const div = document.createElement("div")
        div.className = `arena-card-info`
        div.innerHTML = `<span>En la barra de PS n.º${i} depleted</span><div>${skill[areas[ttdata].skills[i]].info()}</div>`
        document.getElementById("arena-card-effect-list").appendChild(div)
    }
    ...
    if (category == "help") {
        ...
        if (ttdata === `VS`) document.getElementById("tooltipTitle").innerHTML = `Entrenadores VS`
        if (ttdata === `VS`) document.getElementById("tooltipBottom").innerHTML = `¡Derrota a entrenadores cada vez más difíciles y ábrete camino hacia la fama! Irás desbloqueando nuevas zonas que explorar conforme avances`

        if (ttdata === `Frontier`) document.getElementById("tooltipTitle").innerHTML = `Frontera de Combate`
        if (ttdata === `Frontier`) document.getElementById("tooltipBottom").innerHTML = `La Frontera de Combate alberga distintos tipos de retos bajo una restricción de división concreta que rota cada tres días. Los entrenadores se reinician cada día`

        if (ttdata === `Spiral`) document.getElementById("tooltipTitle").innerHTML = `Torre de Combate`
        if (ttdata === `Spiral`) document.getElementById("tooltipBottom").innerHTML = `La Torre de Combate es un reto de dificultad infinita en el que cada Pokémon derrotado la aumenta. Dentro de este reto, las inmunidades de tipo se convierten en resistencias<br><br>Cada vez que entras en la torre empiezas desde la planta 1, pero puedes intentarlo tantas veces como quieras<br><br>Se guardará la planta más alta que alcances, y se reinicia cuando cambia la rotación de liga. Recibirás una recompensa por cada nueva planta más alta al terminar el combate<br><br>Se pueden conseguir un máximo de 100 Chapas Doradas`
        if (ttdata === `Spiral`) document.getElementById("tooltipMid").style.display = `inline`
        if (ttdata === `Spiral`) document.getElementById("tooltipMid").innerHTML = `Rotación de tipo actual: ${format(saved.currentSpiralingType)}`
        ...
        if (ttdata === `BattleArena`) document.getElementById("tooltipBottom").innerHTML = `La Arena de Combate presenta cinco entrenadores cuya dificultad puedes elegir para obtener mejores recompensas. Los efectos de campo y los modificadores de combate también se sortean junto con los entrenadores. Los entrenadores se reinician cada día`

        if (ttdata === `Wild Areas`) document.getElementById("tooltipTitle").innerHTML = `Zonas Salvajes`
        if (ttdata === `Wild Areas`) document.getElementById("tooltipBottom").innerHTML = `Todos los Pokémon de las Zonas Salvajes pueden capturarse derrotándolos. Las Zonas Salvajes rotan cada día, ¡así que no olvides mirar qué se puede capturar hoy!`

        if (ttdata === `Dungeons`) document.getElementById("tooltipTitle").innerHTML = `Mazmorras`
        if (ttdata === `Dungeons`) document.getElementById("tooltipBottom").innerHTML = `Los Pokémon de las Mazmorras no se pueden capturar, pero sueltan objetos útiles y EXP. Las Mazmorras también rotan cada día`

        if (ttdata === `Training`) document.getElementById("tooltipTitle").innerHTML = `Entrenamiento`
        if (ttdata === `Training`) document.getElementById("tooltipBottom").innerHTML = `Enfrenta a tus Pokémon a oleadas de rivales para hacerlos más fuertes. Tendrás ventaja de tipo de forma natural, y su nivel se ajustará al tuyo. Dentro del entrenamiento, las inmunidades de tipo se convierten en resistencias.<br><br>Si fallas un entrenamiento, no obtendrás nada`

        if (ttdata === `Events`) document.getElementById("tooltipTitle").innerHTML = `Eventos`
        if (ttdata === `Events`) document.getElementById("tooltipBottom").innerHTML = `Los eventos pueden contener tanto objetos como Pokémon. Los marcados con una calavera indican rivales poderosos que suelen requerir un objeto para capturarlos (el objeto no se consume si no logras derrotarlos), obtenible en los eventos de recolección. Todos los eventos rotan cada tres días.`

        if (ttdata === `Dimension`) document.getElementById("tooltipTitle").innerHTML = `Megadimensión`
        if (ttdata === `Dimension`) document.getElementById("tooltipBottom").innerHTML = `¡Bienvenido, valiente entrenador, a la última frontera: la Megadimensión, hogar de los Pokémon más fuertes del universo!<br><br>Cada ultraumbral tiene sus propias reglas, con zonas, efectos de campo y destrezas únicos. Además de los efectos de campo, las destrezas se activan al luchar contra el jefe<br><br>Los Pokémon que aparecen en la fisura son aleatorios y cambian en cada rotación`

        if (ttdata === `Genetics`) document.getElementById("tooltipTitle").innerHTML = `Genética`
        if (ttdata === `Genetics`) document.getElementById("tooltipBottom").innerHTML = `Con la genética puedes modificar los parámetros de un Pokémon de nivel 100 (el anfitrión) e influir en ellos usando otro Pokémon (la muestra)<br><br>Al hacerlo, el nivel del anfitrión vuelve a 1 conservando sus 4 movimientos seleccionados, con posibilidad de subir sus IV<br><br>La genética también se puede influir con objetos de genética, que puedes usar al final de la operación<br><br>Encontrarás más información sobre los detalles de la genética en la sección de guía`

        if (ttdata === `Pokerus`) document.getElementById("tooltipTitle").innerHTML = `Pokerus`
        if (ttdata === `Pokerus`) document.getElementById("tooltipBottom").innerHTML = `Cada 12 horas, algunos de tus Pokémon contraerán Pokerus. Este virus es completamente beneficioso y añade un nivel de compatibilidad al Pokémon en genética cuando se usa como anfitrión`

        if (ttdata === `settingsNewPkmn`) document.getElementById("tooltipTitle").innerHTML = `Ajuste`
        if (ttdata === `settingsNewPkmn`) document.getElementById("tooltipBottom").innerHTML = `Oculta automáticamente los Pokémon obtenidos que no sean nuevos tras un combate. Quedan excluidos: Pokémon nuevos, subidas de IV y Pokémon variocolor`

        if (ttdata === `settingsAlternateWild`) document.getElementById("tooltipTitle").innerHTML = `Ajuste`
        if (ttdata === `settingsAlternateWild`) document.getElementById("tooltipBottom").innerHTML = `Reduce en 1 la rotación actual de Zonas Salvajes. Útil si te perdiste la rotación de ayer o si tu horario no encaja con el del juego`

        if (ttdata === `settingsBattleSpeed`) document.getElementById("tooltipTitle").innerHTML = `Velocidad de combate`
        if (ttdata === `settingsBattleSpeed`) document.getElementById("tooltipBottom").innerHTML = `Multiplica el ritmo al que avanza el combate: barras de movimiento, turnos de mejoras y estados, fatiga y reaparición de salvajes<br><br>No cambia ningún cálculo, solo la velocidad a la que ocurren, así que no altera el equilibrio del juego<br><br>El modo AFK ignora este ajuste: al volver, el tiempo acumulado ya se recupera a máxima velocidad`


        if (ttdata === `searchDictionary`) document.getElementById("tooltipTitle").innerHTML = `Términos`
        if (ttdata === `searchDictionary`) document.getElementById("tooltipBottom").innerHTML = `Operadores:<br>![término]: excluir de la búsqueda<br>[términoA] or [términoB]: buscar términoA O términoB<br>[términoA] [términoB]: buscar términoA Y términoB<br><br>Términos de Pokémon:<br>unobtainable, wild, park, event, frontier, mart, shiny, caught, signature, eggMove, [tipo], [habilidad oculta]<br><br>Términos de movimiento:<br>physical, special, signature, [tipo], [habilidad]`
        if (ttdata === `searchDictionary`) document.getElementById("dictionary-search").blur()


        if (ttdata === `searchPokedex`) document.getElementById("tooltipTitle").innerHTML = `Términos`
        if (ttdata === `searchPokedex`) document.getElementById("tooltipBottom").innerHTML = `Operadores:<br>![término]: excluir de la búsqueda<br>[términoA] or [términoB]: buscar términoA O términoB<br>[términoA] [términoB]: buscar términoA Y términoB<br><br>Términos:<br>shiny, pokerus, signature, eggMove, [tipo], [habilidad], [habilidad oculta], [nivel], [movimiento]<br>ivsum(< = >)[número]: suma de los IV (p. ej.: ivsum<36)`
        if (ttdata === `searchPokedex`) document.getElementById("pokedex-search").blur()

        if (ttdata === `Main Challenges`) document.getElementById("tooltipTitle").innerHTML = `Retos principales`
        if (ttdata === `Main Challenges`) document.getElementById("tooltipBottom").innerHTML = `Aquí viven los combates con historia y los retos seleccionados. Pronto habrá más Retos Principales.`

        if (ttdata === `Custom Challenges`) document.getElementById("tooltipTitle").innerHTML = `Retos personalizados`
        if (ttdata === `Custom Challenges`) document.getElementById("tooltipBottom").innerHTML = `Crea combates personalizados con tus propios grupos y equipos rivales. Importa y exporta códigos de reto para compartirlos.`

        if (ttdata === `ModHard`) document.getElementById("tooltipTitle").innerHTML = `Modo difícil`
        if (ttdata === `ModHard`) document.getElementById("tooltipBottom").innerHTML = `Los entrenadores tienen el doble de salud. Solo se aplica a Entrenadores VS`

        if (ttdata === `ModNuzloke`) document.getElementById("tooltipTitle").innerHTML = `Modo Nuzlocke`
        if (ttdata === `ModNuzloke`) document.getElementById("tooltipBottom").innerHTML = `Si cualquiera de tus Pokémon es derrotado combatiendo contra un entrenador de Entrenadores VS, quedará inutilizable de forma permanente hasta que desactives este modo`

        if (ttdata === `ModNoIvs`) document.getElementById("tooltipTitle").innerHTML = `Sin IVs`
        if (ttdata === `ModNoIvs`) document.getElementById("tooltipBottom").innerHTML = `Todos tus Pokémon funcionan con los IV al máximo, sean cuales sean los suyos reales. Puedes desactivarlo sin ningún cambio permanente<br><br>Este modificador no tiene efectos secundarios`

        if (ttdata === `ModAfk`) document.getElementById("tooltipTitle").innerHTML = `AFK más largo`
        if (ttdata === `ModAfk`) document.getElementById("tooltipBottom").innerHTML = `El daño por fatiga se reduce x3<br><br>Este modificador no tiene efectos secundarios`

        if (ttdata === `ModExp`) document.getElementById("tooltipTitle").innerHTML = `Experiencia x3`
        if (ttdata === `ModExp`) document.getElementById("tooltipBottom").innerHTML = `Todos los Pokémon de tu equipo ganan el triple de experiencia al derrotar salvajes<br><br>No afecta a entrenadores, Torre Espiral ni Entrenamiento, que no dan experiencia de por sí`

        if (ttdata === `ModDrops`) document.getElementById("tooltipTitle").innerHTML = `Botín generoso`
        if (ttdata === `ModDrops`) document.getElementById("tooltipBottom").innerHTML = `La probabilidad de que un salvaje derrotado suelte un objeto sube del 20% al 60%<br><br>No afecta a entrenadores ni a zonas que no sueltan objetos`

        if (ttdata === `ModShiny`) document.getElementById("tooltipTitle").innerHTML = `Cazavariocolor`
        if (ttdata === `ModShiny`) document.getElementById("tooltipBottom").innerHTML = `La probabilidad de que un Pokémon salga variocolor sube de 1/400 a 1/40<br><br>Se aplica en el momento de obtenerlo, así que no convierte a los que ya tienes`

        if (ttdata === `ModFatiga`) document.getElementById("tooltipTitle").innerHTML = `Sin fatiga`
        if (ttdata === `ModFatiga`) document.getElementById("tooltipBottom").innerHTML = `Tus Pokémon dejan de perder PS al atacar, así que aguantan indefinidamente en combate<br><br>Sustituye por completo a "AFK más largo", que solo reduce ese daño x3`

        if (ttdata === `ModDureza`) document.getElementById("tooltipTitle").innerHTML = `Equipo resistente`
        if (ttdata === `ModDureza`) document.getElementById("tooltipBottom").innerHTML = `Tu equipo tiene el doble de PS<br><br>Se aplica al entrar en un área, así que hay que volver a entrar para que surta efecto`

        if (ttdata === `ModCruce`) document.getElementById("tooltipTitle").innerHTML = `Cruce constante`
        if (ttdata === `ModCruce`) document.getElementById("tooltipBottom").innerHTML = `La bonificación de Potencia Cruzada (x1.3) se aplica siempre, sin necesidad de alternar tipos entre movimientos<br><br>Libera por completo el orden de la rotación`

        if (ttdata === `curry`) document.getElementById("tooltipTitle").innerHTML = `Curry`
        if (ttdata === `curry`) document.getElementById("tooltipBottom").innerHTML = `Cada 12 h puedes elegir hasta tres habilidades activas para todo tu equipo durante las incursiones, por 1 hora. Siguen las mismas reglas que las habilidades normales y no se acumulan con las que ya tengas`
    ...
    if (category== "ability") {
        ...
        document.getElementById("tooltipTitle").style.display = `inline`
        document.getElementById("tooltipBottom").style.display = `inline`
        document.getElementById("tooltipMid").innerHTML = `Habilidad común`
        if (ability[ttdata].rarity===2) document.getElementById("tooltipMid").innerHTML = `Habilidad poco común`
        if (ability[ttdata].rarity===3) document.getElementById("tooltipMid").innerHTML = `Habilidad rara`
        ...
        if (document.getElementById("pkmn-editor").style.display == "flex" && pkmn[currentEditedPkmn].hiddenAbility?.id == ttdata ) document.getElementById("tooltipBottom").innerHTML += `<br><br>Esta es una habilidad oculta. Se desbloquean con Cápsulas de Habilidad o mediante Entrenamiento, y funcionan junto a tu habilidad normal`
    ...
    if (category=="ribbon") {
        ...
        if (ttdata==`tower1` && pkmn[currentEditedPkmn].recordSpiraling!=undefined) document.getElementById("tooltipMid").innerHTML +=`<br>(Planta más alta alcanzada en la Torre Espiral: ${pkmn[currentEditedPkmn].recordSpiraling})`
        openTooltip()
    }
    ...
    if (category=="move") {
        ...
        let affectedText = ""
        if (affectedAbilities.length>0) affectedText =`<br>Afectado por ${joinWithAnd(affectedAbilities)}`
        if (move[ttdata].restricted) affectedText +=`<br>Este movimiento está restringido (${restrictedIcon}) y solo uno de ellos puede estar en los movimientos activos a la vez`
        ...
        if (move[ttdata].info == undefined) document.getElementById("tooltipBottom").innerHTML = `Sin efectos adicionales`
        ...
        if (document.getElementById("pkmn-editor").style.display == "flex" && pkmn[currentEditedPkmn].signature?.id == ttdata ) document.getElementById("tooltipBottom").innerHTML += `<br><br>Este es un movimiento firma. Los movimientos firma se aprenden automáticamente al nivel 100`
        if (document.getElementById("pkmn-editor").style.display == "flex" && pkmn[currentEditedPkmn].eggMove?.id == ttdata ) document.getElementById("tooltipBottom").innerHTML += `<br><br>Este es un movimiento de huevo. Se pueden aprender mediante genética usando este Pokémon como anfitrión y una muestra que conozca dicho movimiento (normalmente como movimiento firma), con las probabilidades normales de transferencia`
    ...
    if (category=="item") {
        ...
        if (item[ttdata].type==="held" || item[ttdata].heldBonusPower){
            document.getElementById("tooltipTitle").innerHTML = format(ttdata) + `<br>Nivel ${returnItemLevel(ttdata)}`
            document.getElementById("tooltipMid").innerHTML = `${returnItemLevel(ttdata,"stars")}<br>${item[ttdata].got} en total ${returnItemLevel(ttdata,"left")}`
        }

        else document.getElementById("tooltipMid").innerHTML = `${item[ttdata].got} en la bolsa`
    ...
    if (pkmn[poke.id].starsign) document.getElementById("pkmn-editor-sprite").style.filter = `hue-rotate(${starsign[pkmn[poke.id].starsign].hue}deg)`
        ...
        document.getElementById("pkmn-editor-level").innerHTML = `Nivel ${poke.level}`
        ...
        if (pkmn[ttdata].evolve && pkmn[ttdata].evolve()[1].level>0){
            evolutionTag = `<span>Desbloquea ${format(pkmn[ttdata].evolve()[1].pkmn.id)} al nivel ${pkmn[ttdata].evolve()[1].level} ❌</span>`
            if (pkmn[ pkmn[ttdata].evolve()[1].pkmn.id ].caught>0) evolutionTag = `<span>Desbloquea ${format(pkmn[ttdata].evolve()[1].pkmn.id)} al nivel ${pkmn[ttdata].evolve()[1].level} ✔️</span>`
        }
        ...
        if (pkmn[ pkmn[ttdata].evolve()[evo].pkmn.id ].caught>0) evolutionTag += `<span>Desbloquea ${format(pkmn[ttdata].evolve()[evo].pkmn.id)} usando un ${format(item[pkmn[ttdata].evolve()[evo].item.id].id)} al nivel ${levelRequired}+ ✔️</span>`
        else evolutionTag += `<span>Desbloquea ${format(pkmn[ttdata].evolve()[evo].pkmn.id)} usando un ${format(item[pkmn[ttdata].evolve()[evo].item.id].id)} al nivel ${levelRequired}+ ❌</span>`
    ...
    if (pkmn[ttdata].signature) {
        ...
        document.getElementById("pkmn-edit-signature-text").textContent = "Movimiento firma"
        ...
        document.getElementById("pkmn-stats-stat-signature").style.display = "flex"
        document.getElementById("pkmn-edit-signature-text").textContent = "Movimiento de huevo"
...
const sortedMovepool = movepool
  ...
  .sort((a, b) => {
    ...
    if ((areas[saved.currentArea]?.trainer && saved.currentArea != undefined ||  areas[saved.currentArea]?.type == "frontier" ||  saved.currentArea == areas.training.id) && pkmn[ttdata].battling==true) {
        ...
        document.getElementById("tooltipTop").style.display = "none"
        document.getElementById("tooltipBottom").style.display = "none"
        document.getElementById("tooltipTitle").innerHTML = `Acción restringida`
        document.getElementById("tooltipMid").innerHTML = `Ahora mismo los movimientos no se pueden cambiar libremente`
        openTooltip()
        return
    ...
    if (move[moveId].restricted && saved.currentArea != undefined && pkmn[ttdata].battling==true){
        ...
        document.getElementById("tooltipTop").style.display = "none"
        document.getElementById("tooltipBottom").style.display = "none"
        document.getElementById("tooltipTitle").innerHTML = `Movimiento restringido`
        document.getElementById("tooltipMid").innerHTML = `Los movimientos restringidos no se pueden cambiar libremente durante el combate`
        openTooltip()
        return
    }
    ...
    if (category == "dictionaryAbility") {
        ...
        document.getElementById("tooltipTop").style.display = `none`
        document.getElementById("tooltipTitle").innerHTML = format(ttdata)
        document.getElementById("tooltipMid").innerHTML = `Habilidad común<br>${ability[ttdata].info()}`
        if (ability[ttdata].rarity===2) document.getElementById("tooltipMid").innerHTML = `Habilidad poco común<br>${ability[ttdata].info()}`
        if (ability[ttdata].rarity===3) document.getElementById("tooltipMid").innerHTML = `Habilidad rara<br>${ability[ttdata].info()}`
        ...
        if (signatureArray.length>0) signatureText = `<br><br>Además, aparece como habilidad oculta de ${joinWithAnd(signatureArray)}`
        

        document.getElementById("tooltipBottom").innerHTML = `
        La pueden tener los tipos ${joinWithAnd(ability[ttdata].type)} ${signatureText}
        `
        } else {
        ...
        document.getElementById("tooltipBottom").innerHTML = `
        Esta habilidad solo puede aparecer como habilidad oculta de ${joinWithAnd(signatureArray)}
        `
        } else{
        document.getElementById("tooltipBottom").innerHTML = `
        Esta habilidad no se puede obtener por ahora :(
        ` 
        }
   ...
   if (category == "dictionaryMove") {
        ...
        document.getElementById("tooltipBottom").innerHTML = `
        Lo pueden aprender los tipos ${joinWithAnd(move[ttdata].moveset)} ${rarity}
        `
        } else{
        ...
        document.getElementById("tooltipBottom").innerHTML = `
        Este movimiento solo puede aparecer como movimiento firma de ${joinWithAnd(signatureArray)}
        `
        if (eggMoveUsers.length>0) document.getElementById("tooltipBottom").innerHTML += `
        y el movimiento de huevo de ${joinWithAnd(eggMoveUsers)}
        `
    ...
    if (category == "dictionaryPkmn") {
        ...
        let spawnLocation = ""
        if (wildSpawn != "") spawnLocation += `<span>Se encuentra en la zona salvaje ${format(wildSpawn)} (Rotation ${areas[wildSpawn].rotation})</span>`
        if ( wildlifePoolCommon.includes(ttdata) || wildlifePoolUncommon.includes(ttdata) || wildlifePoolRare.includes(ttdata) ) spawnLocation += `<span>Aparece al azar en el Parque Natural</span>`
        if ( exclusiveFrontierPkmn.includes(pkmn[ttdata]) ) spawnLocation += `<span>Se obtiene como recompensa aleatoria en la Frontera de Combate</span>`
        if (encounterSpawn != "") spawnLocation += `<span>Se obtiene en el evento ${format(areas[encounterSpawn].name)} (Rotation ${areas[encounterSpawn].rotation})</span>`
        if (eventSpawn != "") spawnLocation += `<span>Se encuentra en el evento ${format(eventSpawn)} (Rotation ${areas[eventSpawn].rotation})</span>`
        if (spawnLocation == "") spawnLocation = `Este Pokémon no se puede capturar en su fase actual`
        if (pkmn[ttdata].tagObtainedIn == "unobtainable") spawnLocation = `Este Pokémon no se puede obtener`
        if (pkmn[ttdata].tagObtainedIn == "mart") spawnLocation = `Se puede comprar en la Poké Tienda`
        if (pkmn[ttdata].tagObtainedIn == "arceus") spawnLocation = `Se desbloquea al capturar todos los Pokémon obtenibles`
        ...
        if (saved.gamemodAfk !=true) document.getElementById("tooltipMid").innerHTML += `Este Pokémon puede aguantar ${(100 + Math.pow(1.15, 6) * (pkmn[ttdata].bst.hp*30 + (pkmn[ttdata].bst.def + pkmn[ttdata].bst.sdef)*15)*staminaMult).toFixed(0)} turnos antes de debilitarse por Fatiga de Combate con los IV al máximo`
        else document.getElementById("tooltipMid").innerHTML += `Este Pokémon puede aguantar ${(100 + Math.pow(1.15, 6) * (pkmn[ttdata].bst.hp*30 + (pkmn[ttdata].bst.def + pkmn[ttdata].bst.sdef)*15)*staminaMult).toFixed(0)*3} turnos antes de debilitarse por Fatiga de Combate con los IV al máximo`
        ...
        <div onclick="window.getMoveset(pkmn.${ttdata})" class="custom-challenge-button" style="margin-top:0.5rem">Ver movimientos aprendibles</div>
    ...
    if (category == "dictionaryItem") {
        ...
        document.getElementById("tooltipMid").innerHTML = `${item[ttdata].info()}<br>(Tienes ${item[ttdata].got})`
        ...
        for (const i in areas){
            ...
            if (areas[i].drops?.common?.includes(item[ttdata]) || areas[i].drops?.uncommon?.includes(item[ttdata]) || areas[i].drops?.rare?.includes(item[ttdata])){
                obtainText += `<span>Puede caer en la zona salvaje ${format(i)} (Rotation ${areas[i].rotation})</span>`
            }
            }

            if (areas[i].type=="event") {
            if (areas[i].drops?.common?.includes(item[ttdata]) || areas[i].drops?.uncommon?.includes(item[ttdata]) || areas[i].drops?.rare?.includes(item[ttdata])){
                obtainText += `<span>Puede caer en la zona de evento ${format(i)} (Rotation ${areas[i].rotation})</span>`
            }
            }

            if (areas[i].type=="dungeon") {
            if (areas[i].drops?.common?.includes(item[ttdata]) || areas[i].drops?.uncommon?.includes(item[ttdata]) || areas[i].drops?.rare?.includes(item[ttdata])){
                obtainText += `<span>Puede caer en la zona de mazmorra ${format(i)} (Rotation ${areas[i].rotation})</span>`
            }
            }
            ...
            if (areas[i].reward?.includes(item[ttdata])){
                obtainText += `<span>Recompensa del evento ${format(areas[i].name)} (Rotation ${areas[i].rotation})</span>`
            }
            }
        }

        if ([item.hpUp.id, item.protein.id, item.iron.id, item.calcium.id, item.zinc.id, item.carbos.id, item.timeCandy.id].includes(ttdata)) obtainText += `<span>Se obtiene mediante la Recompensa de Exportación</span>`

        for (const i in spiralingRewards) if (spiralingRewards[i].item==ttdata) obtainText += `<span>Recompensa de la Torre Espiral</span>`


        for (const i in shop) {
            if (shop[i].icon == ttdata) obtainText += `<span>Se puede comprar en la Poké Tienda</span>`
        }

        if (item[ttdata].type=="decor" && item[ttdata].rarity) obtainText += `<span>Se puede comprar en la Poké Tienda</span>`




        if (obtainText=="") obtainText += `<span>Este objeto no se puede obtener por ahora</span>`
